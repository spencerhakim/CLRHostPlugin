...
namespace CLROBS {
    ...
    void Texture::SetImage(System::IntPtr data, GSImageFormat imageFormat, unsigned int pitch)
    {
        texture->SetImage(data.ToPointer(), static_cast<::GSImageFormat>(imageFormat), pitch);
    }

    void Texture::SetImage(array<System::Byte>^ data, GSImageFormat imageFormat, unsigned int pitch)
    {
    ...
    System::IntPtr GraphicsSystem::CreateSharedTexture(unsigned int width, unsigned int height, GSColorFormat colorFormat)
    {
    ...
    Texture^ GraphicsSystem::CreateTextureFromSharedHandle(unsigned int width, unsigned int height, System::IntPtr sharedHandle)
    {
        ::Texture *texture = GS->CreateTextureFromSharedHandle(width, height, (HANDLE)sharedHandle.ToPointer());
        return gcnew Texture(texture);
    }

    Texture^ GraphicsSystem::CreateTexture(unsigned int width, unsigned int height, GSColorFormat colorFormat, System::IntPtr data, bool isBuildingMipMaps, bool isStatic)
    {
    ...
    Texture^ GraphicsSystem::CreateTexture(unsigned int width, unsigned int height, GSColorFormat colorFormat, array<System::Byte>^ data, bool isBuildingMipMaps, bool isStatic)
    {
        if (data != nullptr) {
            pin_ptr<unsigned char> dataPtr = &data[0];
    ...
    void GraphicsSystem::DrawSprite(Texture^ texture, unsigned int color, float x, float y, float x2, float y2)
    {
        ::Texture *nativeTexture = static_cast<::Texture *>(texture->OBSTexture.ToPointer());
        GS->DrawSprite(nativeTexture, color, x, y, x2, y2);
    }

    void GraphicsSystem::DrawSpriteEx(Texture^ texture, unsigned int color, float x, float y, float x2, float y2, float u, float v, float u2, float v2)
    {
        ::Texture *nativeTexture = static_cast<::Texture *>(texture->OBSTexture.ToPointer());
        GS->DrawSpriteEx(nativeTexture, color, x, y, x2, y2, u, v, u2, v2);
    }

    void GraphicsSystem::DrawSpriteExRotate(Texture^ texture, unsigned int color, float x, float y, float x2, float y2, float degrees, float u, float v, float u2, float v2, float texDegrees)
    {
        ::Texture *nativeTexture = static_cast<::Texture *>(texture->OBSTexture.ToPointer());
